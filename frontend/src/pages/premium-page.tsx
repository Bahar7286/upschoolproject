...
export default function PremiumPage(): ReactElement {
  ...
  const handleRequest = async () => {
    if (!accessToken) {
      setError(t('premium.loginRequired', 'Talep göndermek için giriş yapmalısınız.'));
      return;
    }
    setLoading(true);
    setError('');
    setMsg('');
    try {
      await submitPremiumRequest(accessToken, t('premium.title', "Premium'a Geç"));
      setPending(true);
      setMsg(t('premium.requestReceived', 'Talebiniz alındı. Admin onayından sonra Premium açılacak.'));
    } catch (err) {
    ...
    <section className="mx-auto max-w-2xl space-y-5 text-center" aria-labelledby="prem-title">
      ...
      <header className="space-y-2">
        ...
        <h1 className="font-display text-3xl font-extrabold tracking-tight text-theme" id="prem-title">
          {t('premium.title', "Premium'a Geç")}
        </h1>
        <p className="text-sm text-theme-muted">
          {t('premium.subtitle', 'Ödeme entegrasyonu yok — Talep Et ile admin onayı alın.')}
        </p>
      </header>
      ...
      <div className="theme-card space-y-3 rounded-2xl p-6 text-left">
        <p className="inline-flex items-center gap-2 text-sm font-bold text-primary-dark dark:text-primary">
          ...
          {t('premium.featuresTitle', 'Premium ile açılanlar')}
        </p>
        <ul className="list-inside list-disc space-y-2 text-sm text-theme-muted">
          <li>{t('premium.feature1', 'Sınırsız AI öneri (ilgi alanına göre rota/plan).')}</li>
          <li>{t('premium.feature2', 'Daha zengin keşif filtreleri ve hızlı öneriler.')}</li>
          <li>{t('premium.feature3', 'Offline paket ve gelişmiş özellikler (yakında).')}</li>
        </ul>
      </div>
      ...
      {isPremium ? (
        <p className="rounded-xl bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200">
          {t('premium.active', 'Premium hesabınız aktif.')}
        </p>
      ) : pending ? (
        <p className="rounded-xl bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900 dark:bg-amber-950/30 dark:text-amber-100">
          {t('premium.pending', 'Talebiniz admin incelemesinde. Onaylandığında burada görünecek.')}{' '}
          <button type="button" className="ml-2 underline" onClick={() => void refreshUser()}>
            {t('premium.refreshStatus', 'Durumu yenile')}
          </button>
        </p>
      ) : (
        <button
        ...
        >
          {loading ? t('premium.submitting', 'Gönderiliyor…') : t('premium.request', 'Talep Et')}
        </button>
      )}
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:justify-center">
        <Link
          ...
          to="/profile"
        >
          {t('premium.backProfile', 'Profile dön')}
        </Link>
        <Link
          className="tap-scale inline-flex min-h-[48px] items-center justify-center rounded-xl border border-stone-900/10 bg-white px-6 font-bold text-stone-900 dark:border-white/10 dark:bg-zinc-900 dark:text-stone-100"
          to="/discover"
        >
          {t('premium.discover', 'Keşfet')}
        </Link>
      </div>
    </section>
  );
