...
export function DistrictRowCard({
  ...
  const displayThumb = useRegionImage(thumb, name, !imageUrl?.trim(), `${citySlug}-${districtSlug}`);
  ...
  return (
    ...
    >
      <RegionThumb src={displayThumb} alt={name} label={name} className="h-16 w-16 shrink-0 rounded-xl object-cover" />
